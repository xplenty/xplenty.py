...
class XplentyClient(object):
    ...
    def get_incremental_clusters(self,since='2013-01-17T22:41:21Z',limit=100,max_retries=1000):
        offset = 0
        method_path ='clusters?offset=' + str(offset) + '&limit='+ str(limit) + '&since=' + str(since)
        response = []
        url = self._join_url(method_path)
        resp = self.get(url)
        response = response + resp
        while len(response) < max_retries:
            offset = offset + limit
            method_path = 'clusters?offset=' + str(offset) + '&limit=' + str(limit) + '&since=' + str(since)
            url = self._join_url(method_path)
            resp = self.get(url)
            ...
            if len(resp) > 1:
                ...
                break
        clusters = Cluster.new_from_dict(response, h=self)
        return clusters
    ...
    def get_incremental_jobs(self,since='2013-01-17T22:41:21Z',limit=100,max_retries=1000):
        offset = 0
        method_path ='jobs?offset=' + str(offset) + '&limit='+ str(limit) + '&since=' + str(since)
        response = []
        url = self._join_url(method_path)
        resp = self.get(url)
        response = response + resp
        while len(response) < max_retries:
            ...
            if len(resp) > 1:
                ...
                break
        jobs = Job.new_from_dict(response, h=self)
        return jobs
    ...
    def get_incremental_packages(self,since='2013-01-17T22:41:21Z',limit=100,max_retries=1000):
        offset = 0
        method_path ='packages?offset=' + str(offset) + '&limit='+ str(limit) + '&since=' + str(since)
        response = []
        url = self._join_url(method_path)
        resp = self.get(url)
        response = response + resp
        while len(response) < max_retries:
            ...
            if len(resp) > 1:
                ...
                break
        packages = Package.new_from_dict(response, h=self)
        return packages
    ...
    @property
    def incremental_clusters(self):
        return self.get_incremental_clusters()
    ...
    @property
    def incremental_jobs(self):
        return self.get_incremental_jobs()
    ...
    @property
    def incremental_packages(self):
        return self.get_incremental_packages()
